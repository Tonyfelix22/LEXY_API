...
class PerformanceReview(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not is_new:
            try:
                old_status = PerformanceReview.objects.get(pk=self.pk).status
        ...
        if self.status == 'SCHEDULED' and old_status != 'SCHEDULED':
            try:
                send_mail(
                    subject=f"Performance Review Scheduled",
                    message=f"Dear {self.employee.first_name},\n\nA performance review has been scheduled for the period: {self.review_period_start} to {self.review_period_end}.\n\nPlease check your dashboard for details.\n\nThank you,\nHR Team",
